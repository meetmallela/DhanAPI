...
# ---------------------------------------------------------------------------
# Basic Auth — credentials stored in MasterConfiguration/config/dashboard_auth.json
# Change username/password there; restart dashboard to pick up changes.
# ---------------------------------------------------------------------------
_AUTH_CFG = Path(MasterResource.MASTER_ROOT) / "config" / "dashboard_auth.json"

def _load_auth_credentials() -> tuple[str, str]:
    """Load (username, password) from dashboard_auth.json."""
    try:
        creds = json.loads(_AUTH_CFG.read_text())
        return creds.get("username", "dhan"), creds.get("password", "")
    except Exception:
        logger.warning("[AUTH] Could not read dashboard_auth.json — auth disabled")
        return "", ""

_AUTH_USER, _AUTH_PASS = _load_auth_credentials()

def _check_auth(username: str, password: str) -> bool:
    """Constant-time comparison to prevent timing attacks."""
    if not _AUTH_USER or not _AUTH_PASS:
        return True   # no credentials configured → open access
    ok_user = hmac.compare_digest(username.encode(), _AUTH_USER.encode())
    ok_pass = hmac.compare_digest(password.encode(), _AUTH_PASS.encode())
    ...
    if auth_header.startswith("Basic "):
        try:
            ...
            if _check_auth(user, pwd):
                return None   # authorised — proceed
        except Exception:
            pass
    return Response(
        "Authentication required",
        401,
        {"WWW-Authenticate": 'Basic realm="Dhan Dashboard"'},
    )
...
def _save(table: str, data):
    ...
    # Filter orders to today only before saving — Dhan sandbox returns the
    # full historical order list (entire account lifetime) on every poll.
    # Without this filter, each 2.5 MB JSON blob is written every 10 s,
    # causing the database to balloon to 1+ GB within weeks.
    ...
    with _conn() as conn:
        ...
        # MySQL does not allow LIMIT inside NOT IN subqueries (error 1235).
        # Wrapping in a derived table (_keep) is the standard workaround.
        conn.execute(
            f"DELETE FROM {table} WHERE id NOT IN "
            f"(SELECT id FROM (SELECT id FROM {table} ORDER BY id DESC LIMIT 500) AS _keep)"
        )
    with _lock:
        _fetch_ts[table] = ts
...
_MARKET_OPEN_H,  _MARKET_OPEN_M  = 9,  10   # IST
_MARKET_CLOSE_H, _MARKET_CLOSE_M = 15, 35   # IST — stop scraping 5 min after close

def _is_market_scrape_window() -> bool:
    """Return True only during market hours (9:10–15:35 IST, weekdays).
    Prevents Dhan sandbox 'Incorrect request' errors after market close."""
    ...
    if now.weekday() >= 5:   # Sat / Sun
        return False
    t = now.time()
    return _t(_MARKET_OPEN_H, _MARKET_OPEN_M) <= t <= _t(_MARKET_CLOSE_H, _MARKET_CLOSE_M)


def _poll_loop():
    """Background thread: hits all Dhan endpoints every POLL_INTERVAL seconds.
    Scraping is limited to market hours (9:10–15:35 IST) to avoid Dhan sandbox
    'Incorrect request for order' errors that flood logs after market close."""
    _scrape_n = 0

    while True:
        if _is_market_scrape_window():
...
def _strategy_pnl_today():
    ...
    try:
        ...
        # Closed trades today grouped by strategy (MCX-OPT excluded — unreliable strikes/P&L)
        cur.execute("""
            SELECT
...
def api_engines():
    ...
    tg_parsers = [
        ...
        {"name": "ShortTerm Parser",        "channels": "INDEX OPTIONS PRIME / COPY MY TRADES BANKNIFTY / STOCK OPTIONS PRIME (×2) / INDEX OPTIONS PRIME (BTST)",
...
@app.route("/api/whatif_calendar")
def api_whatif_calendar():
    """Return daily P&L, capital deployed, and R:R for all available dates.
    Used by the What-If calendar heatmap on the dashboard."""
    try:
        conn = sqlite3.connect(str(MasterResource.get_trading_db_path()), timeout=10)
        rows = conn.execute("""
            SELECT
                ...
                ROUND(SUM(pnl_total), 2)                                       AS pnl,
                ROUND(SUM(entry_price * lot_size), 2)                          AS capital,
                ROUND(AVG(CASE WHEN result='PROFIT' THEN pnl_total   ELSE NULL END), 2) AS avg_win,
                ROUND(AVG(CASE WHEN result='LOSS'   THEN ABS(pnl_total) ELSE NULL END), 2) AS avg_loss
            FROM whatif_trades
            WHERE source = 'EXECUTED' OR source IS NULL
            GROUP BY run_date
            ORDER BY run_date
        """).fetchall()
        conn.close()
        ...
        for r in rows:
            ...
            result.append({
                ...
                "capital": r[5],
                "rr":      rr,
                "wr":      round(r[2] / (r[2]+r[3]) * 100, 1) if (r[2]+r[3]) > 0 else 0,
            })
        return jsonify(result)
    except Exception as e:
        logger.error(f"api_whatif_calendar error: {e}")
        return jsonify([])


@app.route("/api/whatif_rr")
def api_whatif_rr():
    """Return per-strategy R:R, expected value, and capital stats.
    Used by the R:R breakdown table on the dashboard."""
    try:
        conn = sqlite3.connect(str(MasterResource.get_trading_db_path()), timeout=10)
        rows = conn.execute("""
            SELECT
                channel_name                                                         AS strategy,
                COUNT(*)                                                             AS total,
                SUM(CASE WHEN result='PROFIT' THEN 1 ELSE 0 END)                    AS wins,
                SUM(CASE WHEN result='LOSS'   THEN 1 ELSE 0 END)                    AS losses,
                ROUND(AVG(CASE WHEN result='PROFIT' THEN pnl_total     ELSE NULL END), 0) AS avg_win,
                ROUND(AVG(CASE WHEN result='LOSS'   THEN ABS(pnl_total) ELSE NULL END), 0) AS avg_loss,
                ROUND(SUM(pnl_total), 0)                                             AS total_pnl,
                ROUND(SUM(entry_price * lot_size), 0)                                AS capital
            FROM whatif_trades
        ...
        for r in rows:
            ...
            wr  = round(r[2] / (r[2]+r[3]) * 100, 1) if (r[2]+r[3]) > 0 else 0
            ev  = round((wr/100) * aw - (1-wr/100) * (r[5] or 0), 0)
            cap = r[7] or 0
            result.append({
                "strategy":    r[0],
                "total":       r[1],
        ...
        logger.error(f"api_whatif_rr error: {e}")
        return jsonify([])


@app.route("/api/strategy_review")
def api_strategy_review():
    """Return latest EOD strategy review report as JSON for dashboard display."""
    from flask import request as _req
    review_date = _req.args.get("date", datetime.now().strftime("%Y-%m-%d"))
    report_dir = Path(MasterResource.MASTER_ROOT) / "reports"
    report_path = report_dir / f"strategy_review_{review_date}.md"
    if not report_path.exists():
        # Try to find the most recent available report
        reports = sorted(report_dir.glob("strategy_review_*.md"), reverse=True)
        if reports:
            report_path = reports[0]
            review_date = report_path.stem.replace("strategy_review_", "")
        else:
            return jsonify({"error": "No strategy review found. Run eod_strategy_reviewer.py first.",
                            "report": None, "date": None})
    try:
        content = report_path.read_text(encoding="utf-8")
        return jsonify({"report": content, "date": review_date, "error": None})
    except Exception as e:
        return jsonify({"error": str(e), "report": None, "date": None})
    ...
    try:
        ...
        active_rows = con.execute("""
            SELECT symbol, entry_date, entry_price, qty, sl, target, trail_sl,
                   ...
                   DATEDIFF(CURDATE(), entry_date)  AS days_held
        ...
        closed_rows = con.execute("""
            ...
            FROM swing_positions
            WHERE state IN ('CLOSED_PROFIT', 'CLOSED_LOSS', 'CLOSED_TIMEOUT')
              AND exit_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
@app.route("/api/strangle")
def api_strangle():
    """Return kite strangle bot state from strangle_state.json."""
    import json as _json
    state_file = Path(r"C:\Users\meetm\OneDrive\Desktop\GCPPythonCode\DhanAPI\kite_strangle\strangle_state.json")
    try:
        if not state_file.exists():
            return jsonify({"state": "NO_STATE", "session_number": None, "session_history": []})
        data = _json.loads(state_file.read_text(encoding="utf-8"))
        return jsonify(data)
    except Exception as e:
        logger.error(f"api_strangle error: {e}")
        return jsonify({"state": "ERROR", "error": str(e), "session_history": []})
    ...
    try:
        ...
        closed_rows = con.execute("""
            SELECT symbol, add_date, close_date, entry_price, close_price,
              ...
              FROM fa_portfolio
             WHERE status = 'CLOSED'
               AND close_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
        ...
        cleared = con.execute("""
            SELECT symbol, flagged_at, cleared_at, position_qty, avg_price
             ...
             WHERE status = 'CLEARED'
               AND cleared_at >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
...
def api_commodities():
    ...
    try:
        ...
        con.row_factory = sqlite3.Row
        cutoff = (datetime.now() - timedelta(hours=hours)).strftime("%Y-%m-%d %H:%M:%S")
        cutoff48 = (datetime.now() - timedelta(hours=48)).strftime("%Y-%m-%d %H:%M:%S")
...
def api_commodity_paper_trades():
    ...
    try:
        ...
        wins     = _f(agg["wins"])
        losses   = _f(agg["losses"])
        resolved = wins + losses
        win_rate  = round(wins / resolved * 100, 1) if resolved > 0 else None
        total_win_r  = _f(agg["total_win_r"])
        total_loss_r = _f(agg["total_loss_r"])
        exp_val   = round((total_win_r + total_loss_r) / resolved, 2) if resolved > 0 else None
        ...
        return jsonify({
            ...
            "stats": {
                ...
                "open":     int(agg["open_ct"] or 0),
                "wins":     int(wins),
                "losses":   int(losses),
                "expired":  int(agg["expired"] or 0),
                "resolved": int(resolved),
                "win_rate": win_rate,
                "avg_r":    round(_f(agg["avg_r"]),    2) if agg["avg_r"]    else None,
                "avg_win_r":round(_f(agg["avg_win_r"]),2) if agg["avg_win_r"] else None,
                "exp_val":  exp_val,
            },
        })
    except Exception as e:
...
def api_pa_setups():
    ...
    try:
        ...
        where  = ["detected_at >= ?", "confidence >= ?"]
        params: list = [cutoff, conf]
        if dirn != "ALL":
            where.append("direction = ?");  params.append(dirn)
        if status != "ALL":
            where.append("status = ?");     params.append(status)
...
@app.route("/api/confluence_signals")
def api_confluence_signals():
    """
    Today's OmniSignalTrader signals (confluence + pattern-only + PA-only).
    Returns signals grouped by tier with full parsed_data detail.
    """
    try:
        trading_db = str(MasterResource.get_trading_db_path())
        today      = datetime.now().strftime("%Y-%m-%d")
        con        = sqlite3.connect(trading_db, timeout=10)
        con.row_factory = sqlite3.Row

        rows = con.execute("""
            SELECT s.id, s.channel_name, s.raw_text, s.parsed_data,
                   s.timestamp, s.processed,
                   o.order_id, o.status AS order_status, o.entry_price,
                   o.tradingsymbol, o.pnl
            FROM   signals s
            LEFT JOIN orders o ON o.signal_id = s.id
            WHERE  s.channel_id = 'OMNI_SIGNAL_TRADER'
        ...
        result = []
        tier_counts = {"TIER_1_CONFLUENCE": 0, "TIER_2_PATTERN": 0, "TIER_3_PA_ONLY": 0}
        for r in rows:
            try:
                p = json.loads(r["parsed_data"] or "{}")
            except Exception:
                p = {}
            tier = p.get("_tier") or p.get("confidence", "TIER_2_PATTERN")
            ...
            result.append({
                ...
                "pattern":        p.get("_pattern", ""),
                "timeframe":      p.get("_timeframe", ""),
                "pa_setup_type":  p.get("_pa_setup_type", ""),
                "pa_htf":         p.get("_pa_htf", ""),
                "strike":         p.get("strike", ""),
                "strike_label":   p.get("_strike_label", ""),
                "expiry_date":    p.get("expiry_date", ""),
                "dte":            p.get("_dte", ""),
                "tradingsymbol":  r["tradingsymbol"] or p.get("tradingsymbol", ""),
                "spot":           p.get("_spot", ""),
        ...
        logger.error(f"api_confluence_signals error: {e}")
        return jsonify({"signals": [], "tier_counts": {}, "total": 0, "error": str(e)})
...
# ---------------------------------------------------------------------------
# Research Engine v2 -- API endpoints
# ---------------------------------------------------------------------------

import subprocess as _subprocess
import sys as _sys

_RESEARCH_PROC = None   # holds the Popen handle while engine is running
_RESEARCH_LOCK = threading.Lock()
_PYEXE = str(Path(_sys.executable))
_RESEARCH_SCRIPT = str(Path(__file__).parent / "research_engine_v2.py")


@app.route("/api/research/status")
def api_research_status():
    """Return whether the research engine is running + today's paper trade summary."""
    global _RESEARCH_PROC

    # Check process liveness
    with _RESEARCH_LOCK:
        running = _RESEARCH_PROC is not None and _RESEARCH_PROC.poll() is None

    # Query paper trades from MySQL
    try:
        import mysql.connector
        db_cfg = MasterResource.get_db_config()
        conn = mysql.connector.connect(
            host=db_cfg.get("host", "127.0.0.1"),
            port=db_cfg.get("port", 3306),
            user=db_cfg.get("user", "root"),
            password=db_cfg.get("password", ""),
            database=db_cfg.get("database", "trading_live"),
        ...
        closed = [
            {"strategy": r[0], "index": r[1], "n": r[2],
             "wins": int(r[3] or 0), "wr": round(int(r[3] or 0) / r[2] * 100, 1) if r[2] else 0,
             "total_pts": round(float(r[4] or 0), 1)}
            for r in cur.fetchall()
        ]

        # Open trades
        cur.execute(
            """SELECT id, strategy_name, index_name, action, direction,
                      entry_price, current_sl, sl_stage, entry_atr
               FROM research_trades_v2
               WHERE date=%s AND status='OPEN'""",
            (today,)
        )
        open_trades = [
            {"id": r[0], "strategy": r[1], "index": r[2],
             "action": r[3], "direction": r[4],
             "entry": float(r[5] or 0), "sl": float(r[6] or 0),
             "stage": r[7], "atr": float(r[8] or 0)}
            for r in cur.fetchall()
        ]
        ...
        cur.execute(
            """SELECT strategy_name, index_name,
                      COUNT(*) AS n, SUM(win) AS wins,
                      ROUND(AVG(outcome_pts), 1) AS avg_pts
               FROM research_trades_v2
               WHERE date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND status='CLOSED'
               GROUP BY strategy_name, index_name
               ORDER BY strategy_name, index_name""",
        )
        weekly = [
            {"strategy": r[0], "index": r[1], "n": r[2],
             "wins": int(r[3] or 0), "wr": round(int(r[3] or 0) / r[2] * 100, 1) if r[2] else 0,
             "avg_pts": float(r[4] or 0)}
            for r in cur.fetchall()
        ]
        conn.close()
    except Exception as e:
        closed, open_trades, weekly = [], [], []
        logger.warning(f"Research status DB error: {e}")
...
def api_research_start():
    """Start the research engine as a background process."""
    global _RESEARCH_PROC
    with _RESEARCH_LOCK:
        if _RESEARCH_PROC is not None and _RESEARCH_PROC.poll() is None:
            return jsonify({"ok": False, "msg": "Research engine already running"})
        try:
            _RESEARCH_PROC = _subprocess.Popen(
                [_PYEXE, _RESEARCH_SCRIPT],
                creationflags=getattr(_subprocess, "DETACHED_PROCESS", 0),
                close_fds=True,
            )
            logger.info(f"Research engine started (pid={_RESEARCH_PROC.pid})")
            return jsonify({"ok": True, "pid": _RESEARCH_PROC.pid,
                            "msg": "Research engine started"})
        except Exception as e:
...
def api_research_stop():
    ...
    global _RESEARCH_PROC
    with _RESEARCH_LOCK:
        if _RESEARCH_PROC is None or _RESEARCH_PROC.poll() is not None:
            return jsonify({"ok": False, "msg": "Research engine not running"})
        try:
            _RESEARCH_PROC.terminate()
            _RESEARCH_PROC.wait(timeout=10)
            pid = _RESEARCH_PROC.pid
            _RESEARCH_PROC = None
            logger.info(f"Research engine stopped (pid={pid})")
            return jsonify({"ok": True, "msg": "Research engine stopped"})
        except Exception as e:
            return jsonify({"ok": False, "msg": str(e)})
...
if __name__ == "__main__":
    ...
    # Resolve local LAN IP for the startup banner
    ...
    logger.info(f"  Local  : http://127.0.0.1:5050")
    logger.info(f"  LAN    : http://{_lan_ip}:5050")
    logger.info("=" * 60)
    ...
    # Bind to 0.0.0.0 so the dashboard is reachable on all interfaces:
    # localhost, LAN (Wi-Fi/Ethernet), and any static public IP mapped via router port-forward.
