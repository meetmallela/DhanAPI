...
Output:
    MasterConfiguration/reports/Strategy_Praposed_20260531_v105.xlsx
...
OUTPUT = _MASTER / "reports" / "Strategy_Praposed_20260531_v105.xlsx"
...
def write_executive_summary(ws, all_data):
    ...
    title_row(ws, 1, 16,
              "Strategy Assessment v105  |  Final Configuration  |  "
...
# ── Optimization Changes Sheet ────────────────────────────────────────────────

def write_optimization_changes(ws):
    ws.title = "Optimization_Changes"
    ws.sheet_view.showGridLines = False

    title_row(ws, 1, 10,
              "v105 Parameter Changes -- GapFill & ORB15 Optimization Reports (May 31 2026)",
              height=28)

    # ── GapFill ──────────────────────────────────────────────────────────────
    r = 3
    section_title(ws, r, 10, "A.  GapFill Strategy -- Optimization Report Changes", bg="AB47BC")
    r += 1
    for ci, h in enumerate(["Index", "Parameter", "Previous Value", "Optimized Value",
                             "Report WR", "Report Exp", "Rationale", "", "", ""], 1):
        hdr(ws, r, ci, h, bg="7B1FA2")
    r += 1

    gf_changes = [
        ("NIFTY", "Status",          "DISABLED",         "ENABLED",
         "59.4%", "+25.3 pts",
         "Re-enabled with optimized params. Low-vol index actually fills gaps reliably at 0.35%+."),
        ("NIFTY", "Min Gap Size",     "N/A (disabled)",   ">= 0.35%",
         "59.4%", "+25.3 pts",
         "Report #1 rank: 0.35% gap threshold, 69 signals. Excludes sub-0.35% noise gaps."),
        ("NIFTY", "Entry Trigger",    "N/A",              "Immediate Open",
         "59.4%", "+25.3 pts",
         "Immediate open essential for NIFTY -- 5m delay misses most of the fill (low vol index fills fast)."),
        ("NIFTY", "Target",           "N/A",              "60% of gap (partial)",
         "59.4%", "+25.3 pts",
         "Partial 60% fill beats 100% fill: gaps fill 70-80% before institutional block orders reverse."),
        ("NIFTY", "Stop Loss",        "N/A",              "3.0x ATR14",
         "59.4%", "+25.3 pts",
         "Wider SL (3x ATR) gives room for NIFTY intraday noise without premature exit."),
        ("NIFTY", "Hold Window",      "N/A",              "120 minutes",
         "59.4%", "+25.3 pts",
         "NIFTY fills are slower -- 120 min (24 bars) allows full settlement of gap pressure."),

        ("BANKNIFTY", "Gap Size Band","Any gap > 0.3%",   "0.35% to 0.6% BAND",
         "76.9%", "+86.3 pts",
         "Band filter: EXCLUDE gaps >0.6% (trending institutional drives that don't fill). Sweet spot."),
        ("BANKNIFTY", "Entry Trigger","Immediate Open",   "5-min reversal close",
         "76.9%", "+86.3 pts",
         "BANKNIFTY opening drives can run 200-400 pts before reverting -- wait for reversal candle."),
        ("BANKNIFTY", "Target",       "Prev close (100%)","60% of gap (partial)",
         "76.9%", "+86.3 pts",
         "60% partial fill gives highest WR (76.9%). BANKNIFTY rarely gives a 100% clean fill."),
        ("BANKNIFTY", "Stop Loss",    "1.5x ATR",         "1.0x ATR14",
         "76.9%", "+86.3 pts",
         "Tighter 1x ATR SL on BANKNIFTY -- once reversal candle confirms, trade should be directional."),
        ("BANKNIFTY", "Hold Window",  "90 minutes",       "60 minutes",
         "76.9%", "+86.3 pts",
         "BANKNIFTY moves fast -- 60 min (12 bars) is enough. Holding longer adds reversal risk."),
    ]

    for idx, param, old, new, wr, exp, rat in gf_changes:
        fill = _fill("F3E5F5") if idx == "NIFTY" else _fill("EDE7F6")
        cell(ws, r, 1, idx,   fill=fill, bold=True)
        cell(ws, r, 2, param, fill=fill, halign="left")
        cell(ws, r, 3, old,   fill=C_RED_LT if old in ("DISABLED", "N/A", "N/A (disabled)") else None, halign="left")
        cell(ws, r, 4, new,   fill=C_GREEN_LT, halign="left", bold=True)
        cell(ws, r, 5, wr,    fill=C_GREEN_MED, font_colour="FFFFFF", bold=True)
        cell(ws, r, 6, exp,   fill=C_GREEN_LT)
        cell(ws, r, 7, rat,   halign="left")
        ws.row_dimensions[r].height = 28
        r += 1

    r += 1
    section_title(ws, r, 10, "B.  ORB15 Strategy -- Optimization Report Changes", bg="1565C0")
    r += 1
    for ci, h in enumerate(["Index", "Parameter", "Previous Value", "Optimized Value",
                             "Report WR", "Report Exp", "Rationale", "", "", ""], 1):
        hdr(ws, r, ci, h, bg="1976D2")
    r += 1

    orb_changes = [
        ("NIFTY", "ORB Window",      "15 minutes (3x 5m bars)", "5 minutes (first bar only)",
         "60.5%", "+3.75 pts/trade",
         "5m ORB is the top-ranked config. 15m range is too wide -- many mid-range false breakouts."),
        ("NIFTY", "Gap Filter",      "0.5% max gap",            "0.3% max gap (TIGHTER)",
         "60.5%", "+3.75 pts/trade",
         "0.3% gap filter: max drawdown drops from 1232 pts -> 129 pts. The single biggest risk control."),
        ("NIFTY", "Sluggish Cut",    "None (hold 30 min)",      "15-min exit if move < 0.3x ATR",
         "60.5%", "+3.75 pts/trade",
         "Cuts stagnant trades before reversal. Prevents fake-out whipsaws from low-momentum breakouts."),
        ("NIFTY", "TP Target",       "1x ATR (3-bar OR 6-bar)", "1.0x ATR (firm)",
         "60.5%", "+3.75 pts/trade",
         "Consistent 1x ATR target. Report confirms 1.0x ATR optimal for Ultra-Low Drawdown NIFTY."),
        ("NIFTY", "SL",              "Opposite ORB level",      "1.5x ATR (volatility-based)",
         "60.5%", "+3.75 pts/trade",
         "ATR-based SL more appropriate than structural ORB level for 5m bars (5m range can be wide)."),
        ("NIFTY", "Max Drawdown",    "1,232 pts (12M)",         "129 pts (12M)",
         "60.5%", "+3.75 pts/trade",
         "90% reduction in max drawdown. This is the core reason for tightening the gap filter."),

        ("BANKNIFTY", "ORB Window",  "15 minutes",              "5 minutes (first bar only)",
         "62.4%", "+11.92 pts/trade",
         "5m ORB wins on BANKNIFTY too. Shorter range = cleaner breakout signal."),
        ("BANKNIFTY", "Gap Filter",  "0.5% max gap",            "0.3% max gap (TIGHTER)",
         "62.4%", "+11.92 pts/trade",
         "0.3% filter: BANKNIFTY drawdown 4,589 pts -> 495 pts. Essential for live deployment."),
        ("BANKNIFTY", "Sluggish Cut","None",                    "15-min exit if move < 0.3x ATR",
         "62.4%", "+11.92 pts/trade",
         "Same logic as NIFTY. Prevents holding losing positions on low-momentum fake breakouts."),
        ("BANKNIFTY", "TP Target",   "1.5x ATR OR 0.5x ATR",   "1.5x ATR (firm)",
         "62.4%", "+11.92 pts/trade",
         "1.5x ATR for BANKNIFTY (higher vol = bigger target achievable). Report rank #1-#5 all 1.5x."),
        ("BANKNIFTY", "SL",          "Opposite ORB level",      "1.5x ATR (volatility-based)",
         "62.4%", "+11.92 pts/trade",
         "Symmetric TP/SL (1.5x each). Profit factor 1.46 with opposite-range SL."),
        ("BANKNIFTY", "Max Drawdown","4,589 pts (12M)",         "495 pts (12M)",
         "62.4%", "+11.92 pts/trade",
         "89% reduction in max drawdown -- primary safety improvement for live capital."),
    ]

    for idx, param, old, new, wr, exp, rat in orb_changes:
        fill = _fill("E3F2FD") if idx == "NIFTY" else _fill("BBDEFB")
        cell(ws, r, 1, idx,   fill=fill, bold=True)
        cell(ws, r, 2, param, fill=fill, halign="left")
        cell(ws, r, 3, old,   halign="left")
        cell(ws, r, 4, new,   fill=C_GREEN_LT, halign="left", bold=True)
        cell(ws, r, 5, wr,    fill=C_GREEN_MED, font_colour="FFFFFF", bold=True)
        cell(ws, r, 6, exp,   fill=C_GREEN_LT)
        cell(ws, r, 7, rat,   halign="left")
    ...
    r += 1
    section_title(ws, r, 10, "C.  Summary -- Which Preset to Use", bg="2E7D32")
    r += 1
    for ci, h in enumerate(["Preset", "NIFTY ORB WR", "BN ORB WR",
                             "NIFTY ORB DD", "BN ORB DD", "Use When", "", "", "", ""], 1):
        hdr(ws, r, ci, h, bg="388E3C")
    r += 1

    presets = [
        ("Ultra-Low Drawdown (CHOSEN)", "60.5%", "62.4%", "129 pts", "495 pts",
         "Paper trading, early live, minimum stress. WR >60%, tight DD. Applied in v104."),
        ("High Expectancy", "51.9%", "54.6%", "1,109 pts", "2,176 pts",
         "Experienced live traders tolerating deep DD swings for maximum annual points."),
    ]
    for i, row_data in enumerate(presets):
        f = C_GREEN_LT if i == 0 else C_YELLOW
        for ci, v in enumerate(row_data, 1):
            cell(ws, r, ci, v, fill=f, halign="left" if ci in (1, 6) else "center",
                 bold=(ci == 1))
        ws.row_dimensions[r].height = 22
        r += 1

    col_w(ws, [14, 20, 20, 16, 20, 16, 58, 0, 0, 0])
    freeze(ws, 3, 1)
...
def write_reassessment_findings(ws):
    ws.title = 'ReAssessment_v104'
    ws.sheet_view.showGridLines = False
    title_row(ws, 1, 9,
              'v104 Re-Assessment -- Changes Applied in v105  |  May 31 2026', height=28)
    r = 3
    findings = [
        ('GapFill', 'Wed(-36.8pts) and Fri(-55.5pts) massive losers -- disabled both days', 'CRITICAL',
         'All days active', 'Wed + Fri DISABLED for all symbols',
         'NIFTY: WR 55pct->65.6pct, Exp +15->+45pts. SENSEX: Exp -> +109pts', 'APPLIED'),
        ('GapFill BANKNIFTY', '0.35-0.6pct band includes non-filling gaps; sweet spot is 0.5-1.0pct', 'HIGH',
         'Gap band: 0.35-0.6pct', 'Gap band: 0.50-1.00pct',
         'Restricts to high-conviction fills. Report: +30.2 pts test expectancy', 'APPLIED'),
        ('ExpiryBlast', '15th pctile BB filter too strict -- 0 signals in 6M training window', 'HIGH',
         'BB Width < 15th pctile (50-bar)', 'BB Width < 25th pctile (50-bar)',
         'SENSEX: 1 signal fired (100pct WR, +153 pts). Frequency will increase.', 'APPLIED'),
        ('GammaSqueeze', 'Broker RMS auto-square starts at 15:15 IST -- overlaps strategy window', 'EXEC RISK',
         'No exit gate', 'Hard exit at 15:12 IST (code note)',
         'Prevents broker force-exit at wide spreads. Paper-trade only.', 'NOTED'),
        ('ATRSqueeze', 'Volume bypass (spot vol=0) means 1.8x SMA filter never activates', 'MEDIUM',
         'Bypass for spot indices', 'Map to near-month Futures volume (live only)',
         'Futures vol is reliable proxy for spot institutional momentum', 'FUTURE WORK'),
        ('VWAPReclaim', '5-bar confirmation (25 min) may cause entry far from VWAP line', 'LOW',
         '5 bars for BN/SENSEX', 'Monitor fill slippage in paper trading',
         'Review after 20+ live signals', 'MONITOR'),
        ('GapFill BANKNIFTY', 'Overall BN GapFill has negative train expectancy -- SENSEX is primary', 'MEDIUM',
         'BANKNIFTY always active', 'BANKNIFTY: CONDITIONAL (0.5-1.0pct, Mon/Tue/Thu only)',
         'SENSEX GapFill (+109 pts) is the primary engine; BN is supplemental', 'APPLIED'),
    ]
    hdrs = ['Strategy', 'Finding', 'Severity', 'Old Value', 'New Value', 'Expected Impact', 'Status']
    for ci, h in enumerate(hdrs, 1):
        hdr(ws, r, ci, h)
    ws.row_dimensions[r].height = 28
    r += 1
    SEV_FILLS = {
        'CRITICAL':  ('C00000', 'FFFFFF'), 'HIGH': ('FF5722', 'FFFFFF'),
        'EXEC RISK': ('E91E63', 'FFFFFF'), 'MEDIUM': ('FF9800', '000000'), 'LOW': ('FFC107', '000000'),
    }
    STATUS_FILLS = {'APPLIED': C_GREEN_LT, 'NOTED': C_BLUE_LT, 'FUTURE WORK': C_YELLOW, 'MONITOR': C_GREY_LT}
    for strat, finding, sev, old, new, impact, status in findings:
        sb, sf = SEV_FILLS.get(sev, ('DDDDDD', '000000'))
        cell(ws, r, 1, strat,   bold=True, halign='left')
        cell(ws, r, 2, finding, halign='left')
        c = ws.cell(row=r, column=3, value=sev)
        c.fill = _fill(sb); c.font = Font(bold=True, color=sf)
        c.alignment = Alignment(horizontal='center', vertical='center'); c.border = THIN
        cell(ws, r, 4, old,    fill=C_RED_LT,   halign='left')
        cell(ws, r, 5, new,    fill=C_GREEN_LT, halign='left', bold=True)
        cell(ws, r, 6, impact, halign='left')
        cell(ws, r, 7, status, fill=STATUS_FILLS.get(status, C_GREY_LT), bold=True)
        ws.row_dimensions[r].height = 36
        r += 1
    r += 1
    section_title(ws, r, 9, 'Day-of-Week GapFill -- Why Wed and Fri Were Disabled')
    r += 1
    for ci, h in enumerate(['Day', 'Combined BN+SENSEX Expectancy', 'Action', '', '', '', '', '', ''], 1):
        hdr(ws, r, ci, h)
    r += 1
    for dow, exp, action, f in [
        ('Monday', '+41.3 pts', 'ACTIVE', C_GREEN_LT),
        ('Tuesday', '+178.1 pts', 'ACTIVE', C_GREEN_MED),
        ('Wednesday', '-36.8 pts', 'DISABLED', C_RED_LT),
        ('Thursday', '+12.5 pts', 'ACTIVE', C_GREEN_LT),
        ('Friday', '-55.5 pts', 'DISABLED', C_RED_LT),
    ]:
        cell(ws, r, 1, dow, bold=True)
        cell(ws, r, 2, exp, fill=f, bold=True)
        cell(ws, r, 3, action, fill=f, bold=True)
        ws.row_dimensions[r].height = 20
        r += 1
    col_w(ws, [18, 52, 12, 26, 32, 50, 12, 0, 0])
    freeze(ws, 3, 1)


def main():
    print("=" * 60)
    print("  Strategy Assessment v105 -- Building Excel")
    print("=" * 60)
    ...
    print("  Writing Optimization_Changes ...")
    write_optimization_changes(wb.create_sheet("Optimization_Changes"))

    print("  Writing ReAssessment_v104 ...")
    write_reassessment_findings(wb.create_sheet("ReAssessment_v104"))
