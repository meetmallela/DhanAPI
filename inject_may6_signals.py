"""
inject_may6_signals.py
----------------------
Retroactive WhatIf backtest for 2026-05-06.

Engine did NOT run on May 6 (no strategy signals in DB).
Only TG signals from the signals table need processing.

May 6 is Tuesday 2026:
  NIFTY      daily expiry  = 2026-05-06  (same day; token may be expired — bhavcopy fallback)
  BANKNIFTY  weekly expiry = 2026-05-08  (Thursday)
  SENSEX     weekly expiry = 2026-05-07  (Wednesday)
  BANKEX     weekly expiry = 2026-05-07  (Wednesday)
...
# May 6 = Tuesday 2026
#   NIFTY      daily  = 2026-05-06  (same day; bhavcopy fallback if token gone)
#   BANKNIFTY  weekly = 2026-05-08  (Thursday)
#   SENSEX     weekly = 2026-05-07  (Wednesday)
#   BANKEX     weekly = 2026-05-07  (Wednesday)
#   SENSEX50   weekly = 2026-05-07  (Wednesday)
#   FINNIFTY   weekly = 2026-05-12  (next Tuesday)
#   MIDCPNIFTY monthly= 2026-05-26
_TG_EXPIRY_CANDIDATES: dict[str, list[str]] = {
    "NIFTY":      ["2026-05-06", "2026-05-08", "2026-05-12", "2026-05-26"],
    "BANKNIFTY":  ["2026-05-08", "2026-05-29"],
    "FINNIFTY":   ["2026-05-12", "2026-05-19", "2026-05-26"],
    "MIDCPNIFTY": ["2026-05-26"],
    "SENSEX":     ["2026-05-07", "2026-05-28"],
    "BANKEX":     ["2026-05-07", "2026-05-28"],
    "SENSEX50":   ["2026-05-07", "2026-05-28"],
}
...
def _get_tg_signals_for_date(date_str: str) -> list[dict]:
    ...
    for sig_id, ts_raw, channel, pd_raw in rows:
        ...
        if symbol in _SKIP_SYMBOLS or symbol not in _TRADEABLE_SYMBOLS:
            print(f"  SKIP #{sig_id}: symbol={symbol!r}")
            continue
        if not strike or not opt_type:
            print(f"  SKIP #{sig_id}: missing strike={strike} or opt_type={opt_type!r}")
            continue
        if ts >= market_close:
            print(f"  SKIP #{sig_id}: post-market at {ts.strftime('%H:%M')}")
            continue
...
def process_tg_signals(results: list[dict], now_str: str) -> tuple[int, int, int]:
    ...
    print(f"\n[TG] Processing {len(raw)} tradeable TG signals ...\n")
    ...
    for s in raw:
        ...
        print(
            ...
            f"  capital=₹{capital:,.0f}  ROC={roc_pct:+.1f}%  expiry={expiry}"
        )
        results.append({
            "run_date": RUN_DATE, "signal_id": sig_id, "symbol": sym,
            "tradingsymbol": tradingsym, "channel_name": s["channel"],
...
def main():
    ...
    print(f"[May 6 WhatIf] Engine did not run — processing TG signals only.")
    ...
    front_cols = ["run_date","signal_id","channel_name","tradingsymbol","action",
                  ...
                  "pnl_per_unit","pnl_pct","pnl_total","roc_pct","result","data_quality","expiry_date"]
    rest = [c for c in df_out.columns if c not in front_cols]
    df_out = df_out[front_cols + rest]
    df_out.to_csv(csv_path, index=False)
    print(f"[CSV] {csv_path}")

    good = [r for r in results if r["data_available"] == 1]
    profit = sum(1 for r in good if r["result"] == "PROFIT")
    loss   = sum(1 for r in good if r["result"] == "LOSS")
    even   = sum(1 for r in good if r["result"] == "BREAKEVEN")
    net    = sum(r["pnl_total"] for r in good)

    print()
    print("=" * 65)
    print(f"  MAY 6 WHAT-IF SUMMARY — {RUN_DATE}")
    print("=" * 65)
    print(f"  TG signals processed   : {len(results)}")
    print(f"  Kite 1-min [K]         : {tg_k}")
    print(f"  Bhavcopy [B]           : {tg_b}")
    print(f"  No data                : {tg_nd}")
    print(f"  Profit / Loss / Even   : {profit}/{loss}/{even}")
